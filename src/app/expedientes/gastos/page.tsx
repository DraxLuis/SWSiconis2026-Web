...
export default function ExpedientesGastosPage() {
  ...
  const [fase, setFase] = useState('D'); // D = Devengado default
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (searchProveedor) params.append('proveedor', searchProveedor);
      ...
      const res = await fetch(`/api/expedientes/gastos?${params}`);
  ...
  }, [fase, mesDesde, mesHasta, searchProveedor, searchQuery, incluirGenerica, page, filterFteFinanc, filterTipOp, filterMeta, filterGenerica, filterClasificador]);

  // Reset pagination on filter change
  useEffect(() => {
    setPage(1);
  }, [fase, mesDesde, mesHasta, searchProveedor, searchQuery, incluirGenerica, filterFteFinanc, filterTipOp, filterMeta, filterGenerica, filterClasificador]);
  ...
  const exportExcel = () => {
    const data = rows.map(r => ({
      ...
      'Monto': r.monto,
      'Glosa': r.glosa,
      'Proveedor RUC': r.proveedor_ruc,
      'Proveedor Nombre': r.proveedor_nombre
    }));
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Expedientes Gastos');
    XLSX.writeFile(wb, `SWSiconis_Expedientes_Gastos_2026.xlsx`);
  };
        ...
        <div className="bg-[#0c1938] border-b border-slate-700 px-4 py-2 flex items-center justify-between select-none">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-black tracking-wider text-slate-400 uppercase">
              Consulta de Expedientes de Gastos
            </span>
          </div>
          <div className="text-xs font-bold text-[#f87171] bg-red-950/40 border border-red-900/60 rounded px-2.5 py-0.5">
        ...
        <div className="p-4 bg-[#0a1426] border-b border-slate-800 space-y-4 select-none">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-5">
              ...
              <div className="flex items-center gap-2 text-xs font-bold text-slate-300">
                ...
                <select
                  ...
                  className="bg-[#070e1b] border border-slate-700 rounded px-3 py-1 text-red-400 font-mono text-[11px] focus:outline-none focus:border-red-500 font-bold"
                >
              ...
              <div className="flex items-center gap-2 text-xs font-bold text-slate-300">
                ...
                <select 
                  ...
                  className="bg-[#070e1b] border border-slate-700 rounded px-2.5 py-1 text-slate-200 focus:outline-none focus:border-red-500 font-mono text-[11px]"
                >
              ...
              <div className="flex items-center gap-4 text-xs font-bold text-slate-300">
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input 
                    ...
                    className="rounded border-slate-700 bg-[#070e1b] text-red-600 focus:ring-0 focus:ring-offset-0"
                  />
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="flex items-center gap-1.5 text-[10px] font-extrabold bg-[#991b1b] hover:bg-[#801414] text-white rounded px-3 py-1.5 transition-all disabled:opacity-50"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3.5 border-t border-slate-800/40">
            ...
            <div className="flex flex-col gap-1.5">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Buscador General (Expediente / Documento / Glosa / Clasificador)</span>
              <input 
                type="text" 
                placeholder="Escriba término para buscar..."
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="bg-[#070e1b] border border-slate-700 rounded px-3 py-2 text-slate-200 focus:outline-none focus:border-red-500 font-mono text-xs w-full placeholder:text-slate-600"
              />
            </div>

            {/* Buscar RUC / Proveedor */}
            <div className="flex flex-col gap-1.5">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Filtrar por RUC o Nombre del Proveedor</span>
              <input 
                type="text" 
                placeholder="Escriba RUC o nombre del proveedor..."
        ...
        {verFiltro && (
          <div className="p-4 bg-[#091223] border-b border-slate-800 select-none animate-in slide-in-from-top duration-200">
            <div className="border border-slate-700 rounded-lg p-4 relative pt-5 bg-[#050b16]">
              ...
              <span className="absolute -top-3 left-4 bg-[#091223] px-2.5 text-xs font-black text-red-400 border border-slate-700 rounded uppercase tracking-wider">
                Filtro de Consulta
              </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-4 items-end">
                ...
                <div className="space-y-1.5">
                  ...
                  <select 
                    ...
                    className="w-full text-[11px] bg-[#070e1b] border border-slate-700 rounded px-2.5 py-1.5 text-slate-200 focus:outline-none focus:border-red-500 font-mono font-semibold"
                  >
              ...
              <div className="flex justify-end gap-3 mt-4 pt-2 border-t border-slate-800/60">
                ...
                <button
                  ...
                  className="px-4 py-1.5 text-[10px] font-extrabold bg-[#991b1b] hover:bg-[#801414] text-white rounded transition-all"
                >
                  Aplicar Filtro
                </button>
              </div>
          ...
          <table className="min-w-[1500px] w-full text-left border-collapse table-fixed">
            <thead className="sticky top-0 bg-[#0c182e] border-b border-slate-700 text-slate-400 text-[10px] uppercase font-black tracking-wider z-20 select-none">
              <tr>
                ...
                <th className="py-2.5 px-3 w-[250px]">Proveedor</th>
              ...
              {loading ? (
                Array.from({ length: 8 }).map((_, idx) => (
                  <tr key={idx} className="animate-pulse">
                    <td colSpan={16} className="py-3 px-4"><div className="h-4 bg-slate-800 rounded w-full" /></td>
                  </tr>
                ))
              ) : rows.length === 0 ? (
                <tr>
                  <td colSpan={16} className="py-24 text-center text-slate-500 font-bold">
                    <div className="flex flex-col items-center justify-center gap-2">
                      <AlertTriangle className="h-8 w-8 text-slate-600" />
                      <span>No se encontraron expedientes de gastos que coincidan con la búsqueda.</span>
                    </div>
                  </td>
                </tr>
              ) : (
                rows.map((r, idx) => (
                  <tr key={`${r.expediente}-${r.sec_reg}-${idx}`} className="hover:bg-[#112240] transition-colors font-mono text-[11px]">
                    ...
                    <td className="py-2 px-2 text-center text-red-400 font-bold">{r.fase}</td>
                    <td className="py-2 px-3 text-right font-bold text-[#f87171]">{fmt(r.monto)}</td>
                    <td className="py-2 px-3 truncate" title={r.proveedor_nombre}>{r.proveedor_nombre || '—'}</td>
        ...
        <div className="bg-[#0c1938] border-t border-slate-700 px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 select-none">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 text-sm font-black text-slate-300 bg-slate-950/60 border border-slate-800 px-4 py-1.5 rounded-lg">
              <span>TOTAL S/:</span>
              <span className="font-mono text-[#f87171] text-base">{fmt(totalMonto)}</span>
            </div>
          </div>
        </div>
