...
import { exportComprobantesPago } from '@/lib/excel-exports';
...
export default function PagosPage() {
  ...
  const [selectedRow, setSelectedRow] = useState<PagoRow | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Auto-select first row if nothing is selected or if rows changed
        if (data.rows.length > 0) {
          // If previous selection exists in new rows, keep it
          const exists = data.rows.find((r: PagoRow) => r.expediente === selectedRow?.expediente && r.sec_reg === selectedRow?.sec_reg && r.corr === selectedRow?.corr);
          if (exists) {
  ...
  useEffect(() => {
    fetchData();
  }, [page, filterMes, filterTipoOp, filterRubro, fetchData]); // re-fetch on filter/page change
  ...
  const handleExportExcel = async () => {
    try {
      ...
      if (data.success && data.rows) {
        exportComprobantesPago(data.rows);
      }
    } catch (e) {
      console.error('Error exporting payments:', e);
    }
  };
      ...
      <div className="w-full rounded-xl border border-slate-700 bg-[#070e1b] shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="bg-[#a7f3d0] text-[#064e3b] px-4 py-2.5 flex items-center justify-between shadow-sm select-none">
          <h2 className="font-extrabold text-sm tracking-wide uppercase flex items-center gap-2">
            Comprobantes de Pago{filterAno ? ` - Año ${filterAno}` : ' - Todos los Períodos'}
          </h2>
          <button 
            onClick={fetchData}
        ...
        <div className="p-4 bg-[#0a1426] border-b border-slate-800 space-y-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="relative w-full md:w-80">
              <input
                type="text"
                placeholder="Buscar por Expediente, Doc, RUC..."
                value={search}
        ...
        <div className="w-full max-w-full overflow-x-auto bg-[#080f1d] min-h-[450px]">
          <table className="min-w-[1145px] w-full text-left border-collapse table-fixed">
            <thead className="sticky top-0 bg-[#0c182e] border-b border-slate-700 text-slate-400 text-[10px] uppercase font-black tracking-wider z-20 select-none">
              <tr>
                ...
                <th className="py-2.5 px-2 w-[40px] text-center">TOp</th>
                ...
                <th className="py-2.5 px-2 w-[140px]">Num.Doc.</th>
              ...
              {loading ? (
                Array.from({ length: 10 }).map((_, idx) => (
                  <tr key={idx} className="animate-pulse">
                    <td colSpan={14} className="py-3 px-4"><div className="h-4 bg-slate-800 rounded w-full" /></td>
                  </tr>
                ))
              ) : rows.length === 0 ? (
                <tr>
                  <td colSpan={14} className="py-24 text-center text-slate-500 font-bold">
                    <div className="flex flex-col items-center justify-center gap-2">
                      <AlertTriangle className="h-8 w-8 text-slate-600" />
                      <span>No se encontraron comprobantes de pago registrados.</span>
                    </div>
                  </td>
                </tr>
              ) : (
                rows.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 px-2 text-center font-mono text-[11px]">{r.tipo_op}</td>
                      ...
                      <td className="py-2 px-2 truncate font-mono text-[11px]" title={r.num_doc}>{r.num_doc}</td>
