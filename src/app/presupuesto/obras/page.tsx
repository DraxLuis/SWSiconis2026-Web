...
export default function ObrasPage() {
  ...
  const [selectedRow, setSelectedRow] = useState<ObraRow | null>(null);
    ...
    try {
      ...
      if (data.success) {
        ...
        if (fetchedRows.length > 0) {
          const match = fetchedRows.find((r: ObraRow) => r.act_proy === selectedRow?.act_proy);
          if (match) {
            setSelectedRow(match);
          } else {
            setSelectedRow(fetchedRows[0]);
          }
        } else {
          setSelectedRow(null);
        }
  ...
  }, [filterRubro, selectedRow?.act_proy]);
  ...
  useEffect(() => {
    if (displayRows.length > 0) {
      const match = displayRows.find(r => r.act_proy === selectedRow?.act_proy);
      if (!match) {
  ...
  }, [page, search, rows, displayRows, selectedRow?.act_proy]);
  ...
  const handleExportObras = async () => {
    ...
    try {
      const mapped = filteredRows.map(r => ({
        act_proy: r.act_proy,
        act_proy_nombre: r.act_proy_nombre,
        pia: r.pia,
        pim: r.pim,
        certif: r.certif,
      ...
      exportEjecucionActProy(mapped, 'ejecucion-obras.xlsx', 'Ejecución Presupuestal  - Componente / Obra / Acc. Inv.');
    } catch (e) {
      console.error(e);
    } finally {
      setExporting(false);
  ...
  // Export Ficha Proyecto/Obra
  const handleExportFicha = async () => {
    if (!selectedRow) return;
    setExporting(true);
    try {
      const res = await fetch(`/api/proyectos/ficha?codigo=${selectedRow.act_proy}`);
      const data = await res.json();
      if (data.success) {
        exportFichaProyecto(data.projectInfo, data.classifiers);
      } else {
        alert(data.error || 'No se pudo generar la ficha resumen.');
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  // Selected Obra Exports
  const handleExportSelected = async (actionType: 'certificados' | 'devengados' | 'giros' | 'giros_glosa') => {
    if (!selectedRow) return;
    setExporting(true);
    try {
      const sfs = selectedRow.sec_funcs || [];
      if (sfs.length === 0) {
        alert('Esta obra no tiene metas asociadas para exportar.');
        return;
      }
      ...
      if (actionType === 'certificados') {
        ...
        if (data.success && data.rows) {
          const filtered = data.rows
            .filter((c: CertificadoReportRow) => c.sec_func && sfs.includes(c.sec_func))
          ...
          exportCertificados(selectedRow.act_proy, selectedRow.act_proy_nombre, filtered, `certificados_obra_${selectedRow.act_proy}.xlsx`);
        }
        ...
        if (data.success && data.rows) {
          const filtered = data.rows
            .filter((d: DevengadoReportRow) => d.sec_func && sfs.includes(d.sec_func))
          ...
          exportDevengados(selectedRow.act_proy, selectedRow.act_proy_nombre, filtered, `devengados_obra_${selectedRow.act_proy}.xlsx`);
        }
        ...
        if (data.success && data.rows) {
          const filtered = data.rows
            .filter((g: GiroReportRow) => {
              const sf = g.sec_func ? String(g.sec_func).trim().substring(0, 4) : '';
              return sfs.includes(sf);
            })
            .map((g: GiroReportRow) => ({
              ano_eje: g.ano || '2026',
              mes_eje: g.mes,
          ...
          if (actionType === 'giros') {
            exportGiros(selectedRow.act_proy, selectedRow.act_proy_nombre, filtered, `giros_obra_${selectedRow.act_proy}.xlsx`);
          } else {
            exportGirosConGlosa(selectedRow.act_proy, selectedRow.act_proy_nombre, filtered, `giros_glosa_obra_${selectedRow.act_proy}.xlsx`);
          }
        }
      }
    } catch (e) {
        ...
        <div className="w-full rounded-lg border border-slate-800 bg-[#070e1b] overflow-hidden flex flex-col">
          ...
          <div className="bg-[#0c1938] border-b border-slate-700 px-4 py-2 flex items-center justify-between select-none">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-black tracking-wider text-slate-400 uppercase">
                Consulta de Obras y Acciones de Inversión
              </span>
            </div>
          ...
          <div className="bg-[#fef3c7] text-[#92400e] px-4 py-2.5 flex items-center justify-between shadow-sm select-none">
            <h2 className="font-extrabold text-sm tracking-wide uppercase flex items-center gap-2">
              Ejecución Presupuestal - Obras y Acciones de Inversión - Año 2026
            </h2>
            <div className="flex items-center gap-2">
              <button 
            ...
            <table className="min-w-[1000px] w-full text-left border-collapse table-fixed">
              <thead className="sticky top-0 bg-[#0c182e] border-b border-slate-700 text-slate-400 text-[10px] uppercase font-black tracking-wider z-20 select-none">
                <tr>
                  ...
                  <th className="py-2.5 px-2 w-[100px] text-center">Código Obra</th>
                  <th className="py-2.5 px-3 w-[350px]">Nombre / Descripción de la Obra</th>
                  ...
                  <tr>
                    <td colSpan={8} className="py-20 text-center text-slate-500 font-bold">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <span>No se encontraron Componentes u Obras registradas.</span>
                      </div>
                    </td>
                  </tr>
                ) : (
                  displayRows.map((r) => {
                    const isSelected = selectedRow?.act_proy === r.act_proy;
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-2 px-2 text-center font-mono text-[11px] font-bold">{r.act_proy}</td>
                        <td className="py-2 px-3 truncate" title={r.act_proy_nombre}>{r.act_proy_nombre}</td>
          ...
          {!loading && filteredRows.length > PAGE_SIZE && (
            <div className="bg-[#0a1426] border-t border-slate-800 px-6 py-2.5 flex items-center justify-between select-none">
              <div className="text-xs text-slate-400 font-mono">
                Mostrando <span className="font-extrabold text-white">{Math.min(filteredRows.length, (page - 1) * PAGE_SIZE + 1)}-{Math.min(filteredRows.length, page * PAGE_SIZE)}</span> de <span className="font-extrabold text-white">{filteredRows.length}</span> registros
              </div>
          ...
          {/* Lower Panel: Selected Detail & Action buttons */}
          <div className="bg-[#0a1426] border-t border-slate-700 p-4 grid grid-cols-1 lg:grid-cols-12 gap-4 items-stretch select-none">
            
            {/* Selected Detailed Box (8 cols) */}
            <div className="lg:col-span-8 flex flex-col border border-slate-800 rounded-lg p-3.5 bg-[#080f1d] text-xs space-y-2">
              <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-1.5 border-b border-slate-800/60 pb-1.5">
                ...
                Especificaciones de la Obra Seleccionada
              </div>
              {selectedRow ? (
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2 text-slate-300 font-mono text-[11px] leading-relaxed">
                  <div className="md:col-span-2 text-slate-500 font-bold">CÓDIGO OBRA:</div>
                  <div className="md:col-span-10 text-white font-extrabold">{selectedRow.act_proy}</div>
                  
                  <div className="md:col-span-2 text-slate-500 font-bold">DESCRIPCIÓN:</div>
                  <div className="md:col-span-10 text-slate-200">{selectedRow.act_proy_nombre}</div>

                  <div className="md:col-span-2 text-slate-500 font-bold">METAS ASOCIADAS:</div>
                  <div className="md:col-span-10 text-[#f59e0b] font-extrabold">{selectedRow.metas_count} metas presupuestales</div>

                  <div className="md:col-span-2 text-slate-500 font-bold">DEVENGADO:</div>
                  <div className="md:col-span-4 text-emerald-400 font-extrabold">{formatMoney(selectedRow.devengado)}</div>

                  <div className="md:col-span-2 text-slate-500 font-bold md:text-right">GIRADO:</div>
                  <div className="md:col-span-4 text-blue-400 font-extrabold">
                    {formatMoney(selectedRow.girado)}
                  </div>
                </div>
              ) : (
                <div className="text-slate-500 text-center py-6 font-bold">
                  Seleccione una obra en la tabla superior para ver su información detallada.
                </div>
              )}
            </div>
            ...
            <div className="lg:col-span-4 flex flex-col justify-between border border-slate-800 rounded-lg p-3.5 bg-[#080f1d] space-y-3">
              <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-800/60 pb-1.5 flex justify-between items-center select-none">
                <span>Reportes por Obra Seleccionada</span>
                {selectedRow && (
                  <span className="font-mono text-[#f59e0b] font-bold">Act.Obra: {selectedRow.act_proy}</span>
                )}
              </div>
              <div className="grid grid-cols-2 gap-2 flex-1 items-center">
                ...
                <button
                  ...
                  disabled={!selectedRow || exporting}
                  className="w-full flex items-center justify-center gap-1.5 text-[11px] font-bold py-2 px-2.5 rounded border border-amber-900 bg-amber-950/40 hover:bg-amber-950/40 text-amber-400 hover:text-amber-300 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  <FileSpreadsheet className="h-3.5 w-3.5" />
                  Giros
                </button>
              ...
              <button
                ...
                disabled={!selectedRow || exporting}
                className="w-full flex items-center justify-center gap-1.5 text-[11px] font-bold py-2.5 px-3.5 rounded border border-[#92400e] bg-amber-950/40 hover:bg-[#b45309]/30 text-amber-400 hover:text-amber-300 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
              >
                <FileSpreadsheet className="h-3.5 w-3.5" />
                Ficha Resumen de Obra
              </button>
            </div>
